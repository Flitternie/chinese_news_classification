...
def train(args):
    ...
    model = model_class.from_pretrained(args.ckpt) if args.ckpt else model_class.from_pretrained(args.model_name_or_path, num_labels=args.num_labels) 
    model.resize_token_embeddings(len(tokenizer))

    if args.local_rank in [-1, 0]:
        logging.info("Create train_loader and val_loader.........")
    train_data, val_data = load_and_split_data(args.input_dir)
    train_loader = prepare_data(args, train_data, tokenizer, training=True, distributed=args.n_gpus>1)    
    val_loader = prepare_data(args, val_data, tokenizer, training=False, distributed=False)   
    ...
    t_total = len(train_loader) * args.num_train_epochs    # Prepare optimizer and schedule (linear warmup and decay)
    ...
    if args.ckpt and args.local_rank in [-1, 0]:
        logging.info("Continuing training from checkpoint: %s", args.ckpt)
            
    tr_loss = 0.0
    best_acc, current_acc = 0.0, 0.0
    model.zero_grad()
    if args.local_rank in [-1, 0]:
        current_acc, _ = validate(args, model, val_loader, device)
